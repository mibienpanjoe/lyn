...
| Cursor workspace window | A local editor integration supplies the exact editor-window correlation and workspace directory | Supported on Linux X11 using the local VSIX and Cursor's X11 window class. |
| Browser tab (Chrome, Chromium, Brave, Edge, Firefox) | Companion WebExtension + Native Messaging helper correlates active tab URL; localhost dev-server port maps to process cwd | Supported on Linux X11 via `lyn-browser-host` and localhost port mapping; incognito tabs and query/auth tokens are excluded. |
