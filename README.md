...
The implemented shell uses Tauri 2, Rust, Svelte 5, TypeScript, Vite, and bundled SQLite through `rusqlite`. Rust owns the shared domain and IPC primitives, generated TypeScript bindings, database initialization, canonical schema, transactional migrations, context and text/screenshot capture repositories, native project-directory selection, capture-session lifecycle, and recoverable Lyn-owned media staging. Screenshot paste, opaque preview, optional manual caption, replacement, cancellation, and durable save are implemented; voice capture remains a subsequent slice.
