...
The implemented shell uses Tauri 2, Rust, Svelte 5, TypeScript, Vite, and bundled SQLite through `rusqlite`. Rust owns the shared domain and IPC primitives, generated TypeScript bindings, database initialization, canonical schema, transactional migrations, context and text/media capture repositories, native project-directory selection, capture-session lifecycle, recoverable Lyn-owned media staging, microphone input, and audio playback. Screenshot paste and voice recording expose only opaque media IDs to the popup; both support optional manual captions, replacement, removal back to the preserved text draft, cancellation, retry, and durable save.
