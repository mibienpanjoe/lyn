...
Lyn has a Svelte/Tauri implementation; `README.md` is the entry point. Read `docs/project_overview.md`, then `01_requirements_prd.md` through `09_local_speech_distribution_decision.md`. Preserve requirement, invariant, and error IDs across documents.

Svelte 5/TypeScript UI lives under `src/`; the Tauri/Rust core lives under `src-tauri/`. Window IPC is gated by `src-tauri/capabilities/` (`main` Library vs `capture` popup) referencing permission sets in `src-tauri/permissions/` plus `build.rs` `AppManifest::commands` allowlists—never `core:default` path/image escape hatches. Separately delivered local context integrations live under `integrations/`: VS Code reports focus plus local workspace folders, the Kitty watcher reports exact pane focus plus child process identity, and the generic shell bootstrap starts the bounded `lyn-context` helper (Lyn refreshes that helper next to the app on launch so users do not build a second binary). Native Messaging uses the same running binary via `~/.local/share/lyn/bin/lyn-browser-host`. All provider traffic uses private Rust-owned sockets and excludes commands, output, environment values, and client-supplied paths. 1-click integration status checks and installations are managed under `src-tauri/src/integrations/` and exposed via `commands::integrations` to the Library window. Ordered SQLite migrations live under `src-tauri/migrations/` and must remain immutable after release. Keep presentation and IPC clients in `src/`; storage, capture-session state, context detection, media, and OS integration belong in Rust. Capture-session transitions live in `src-tauri/src/capture/session.rs`; provider observations and their ephemeral registry live under `src-tauri/src/context/`; command gateways live under `src-tauri/src/commands/`, parameterized context/capture persistence stays under `src-tauri/src/storage/`, and platform-specific shortcut/focus/audio/popup-layout adapters stay behind `src-tauri/src/platform/` ports. Recoverable media staging is in `src-tauri/src/media/staging.rs`; PNG/WAV normalization stays under `src-tauri/src/media/`, and startup reconciliation reads durable media references through `src-tauri/src/storage/media_assets.rs`. Staged previews resolve through the read-only `lyn-media://` protocol by opaque ID; the capture/library CSP allows that scheme (and the Windows `lyn-media.localhost` form) on `img-src`/`media-src` only. None of these boundaries may expose raw filesystem paths to the UI.
...
- `pnpm provider:browser:build` — build the standalone `lyn-browser-host` binary for isolated helper debugging (not required for users; Lyn refreshes `~/.local/share/lyn/bin/lyn-browser-host` from its own binary).
...
- `pnpm provider:terminal:build` — build the standalone `lyn-context` binary for isolated helper debugging (not required for users; Lyn's own binary handles `watch --process`).
